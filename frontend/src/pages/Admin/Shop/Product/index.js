...
function ProductsManagePage() {
  ...
  useEffect(() => {
    setIsLoading(true);
    AuthRequest.get("premises/shop/" + id)
      .then((response) => response.data)
      .then((data) => {
        if (data) {
          setDataPermise(data);
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: (a, b) => a?.type[0]?.quantity - b?.type[0]?.quantity,
      sortDirections: ["descend", "ascend"],
      render: (_, record) => <div>{record?.type[0]?.quantity || "Trống "}</div>,
    },
    {
      title: "Giảm giá",
      dataIndex: "discount",
          ...
          <span
            ...
            onClick={(e) => {
              if (record.type.length > 0) {
                ...
                Toast(0, "Kiểu loại sản phẩm đang trống!");
              }
            }}
          >
            Chi tiết
  ...
  const hanldeDeleteImage = (filename) => {
    setIsLoading(true);
    AuthRequest.delete("products/file/" + dataForm.id, {
      filename: filename,
    })
      .then((response) => response.data)
      .then((data) => {
          ...
          <div className="p-6 z-0">
            <div className="flex justify-between items-center">
              <div className="mb-4 font-bold text-5xl">
                <i>Qt sản phẩm cửa hàng {dataPermise.name}</i>
              </div>
              <div
