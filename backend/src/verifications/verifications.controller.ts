import { Controller, Get, Post, Body, Param, Res, NotFoundException, BadRequestException, HttpException } from '@nestjs/common';
...
export class VerificationsController {
  ...
  async onVerify(@Param('id') id: string, @Param('code') code: number, @Body() password?: { password?: string }) {
    console.log(id, code, password);
    try {
      if (password.password) {
        return await this.verificationsService.handleVerifyEmail(id, code, password?.password);
      } else {
  ...
  async onRefresh(@Param('id') id: string) {
    try {
      ...
      throw new HttpException(
        {
          message: e ? 'Có lỗi sảy ra xin vui lòng thử lại sau.' : 'TokenID đã hết hạn vui lòng đăng ký lại.!',
        },
        404,
      );
    }
