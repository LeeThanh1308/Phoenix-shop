...
export class VerificationsService {
  ...
  handleDecodeId(id: string): any {
    return new Promise(async (resolve) => {
      try {
        const decryptedData = await CryptoJS.AES.decrypt(CryptoJS.enc.Utf8.stringify(CryptoJS.enc.Hex.parse(id)), process.env.CRY_KEY).toString(
          CryptoJS?.enc?.Utf8,
        );
        if (decryptedData) {
  ...
  async create(createVerificationDto: CreateVerificationDto) {
    ...
    try {
      ...
      const { id, email, code } = await queryRunner.manager.save(Verifications, results);
  ...
  async handleClearVerify() {
    ...
    timeExpVerify.setSeconds(timeExpVerify.getSeconds() - Number(process.env.TIME_VERIFY_SECOND));
    await this.verificationsRepository.createQueryBuilder().delete().where('createdAt <= :date', { date: timeExpVerify }).execute();
  }
  ...
  async handleVerifyEmail(id: any, verifyCode: number, password?: string) {
    ...
    if (!dataVerify) {
      if (handleCheckId.total_verify > 0) {
        await this.verificationsRepository.update({ id: id }, { total_verify: --handleCheckId.total_verify, isSuccess: false });
      }
      throw new Error();
    }
    if (dataVerify.data) {
      const resultCreAcc = await this.accountService.handleCreateAccount({ email: dataVerify.email, ...dataVerify.data }, false, Number(id));
      if (!resultCreAcc) {
        throw new Error();
      }
    ...
    if (password && handleCheckId?.forget_password) {
      await this.verificationsRepository.update({ id: id }, { isSuccess: true });
      const result = await this.accountService.handleUpdateForgetPass({ id: id, password });
      if (!result) {
        throw new Error();
      }
      return {
  ...
  async handleRefreshCode(id: any) {
    ...
    curremtDate.setSeconds(curremtDate.getSeconds() - Number(process.env.TIME_REFRESH_SECOND));
    ...
    if (!onFreshToken.forget_password) {
      const result = await this.handleSendEmail(onFreshToken.email, codeVerify, onFreshToken.data.fullname);
      if (!result) {
        throw new Error();
      }
    } else {
      const result = await this.handleSendEmailForget(onFreshToken.email, codeVerify);
      if (!result) {
        throw new Error();
      }
    }
    ...
    return { message: 'Refresh code thành công!', data: await this.handleCheckRefreshCode(id) };
  }
  ...
  async handleCheckRefreshCode(id: string) {
    ...
    const expRefreshToken = differenceInSeconds(
      result.updatedAt,
      curremtDate.setMinutes(curremtDate.getMinutes() - +process.env.TIME_REFRESH_SECOND / 60),
    );
    const expVerify = differenceInSeconds(
      result.createdAt,
      curremtVerify.setMinutes(curremtVerify.getMinutes() - +process.env.TIME_VERIFY_SECOND / 60),
    );
    return {
  ...
  async handleChangePassword(id: number) {
    await this.handleClearVerify();
    const result = await this.verificationsRepository.findOne({ where: { id: id, isSuccess: true, total_verify: MoreThan(0) }, select: ['email'] });
    if (!result) {
      return false;
    }
    await this.verificationsRepository.delete({ id: id });
